...
export default function Home() {
  ...
  return (
    <div className={styles.container}>
      ...
      <Header onClose={(e: { target: { value: SetStateAction<string>; }; }) => setSearch(e.target.value)} />
      <section className={styles.card}>
        {resposta &&
          Object.values(resposta).map((characters: any) => {
            return (
              <Cards data={characters} key={characters.id} />
            )
          })}
      </section>
      ...
      <div className={styles.contant}>
        {page > 1 &&
          <button className={styles.avan} onClick={() => setPage(page - 1)}>
            <ArrowRight />
          </button>
        }
        {page < 42 &&
          <button className={styles.prox} onClick={() => setPage(page + 1)}>
            <ArrowRight />
          </button>
        }
      </div>
